...
import com.uqam.mentallys.databinding.FragmentAddEventMaintenanceBinding
...
@AndroidEntryPoint
class AddEventMaintenanceFragment : Fragment(R.layout.fragment_add_event_maintenance) {

    private lateinit var binding: FragmentAddEventMaintenanceBinding
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding = FragmentAddEventMaintenanceBinding.bind(view)
        ...
        binding.apply {
            ...
            // Get Context data
            dropDownEventContext.setOnItemClickListener { parent, view, position, id ->
...
//                Log.d("tag_name", tag.toString())
            ...
            viewModel.addEditEvent.collect { eventFromChannel ->
                when (eventFromChannel) {
                    is AddEditEventViewModel.AddEditPathwayEvent.NavigateToSavedEventScreen -> {
                        val action =
                            AddEventMaintenanceFragmentDirections.actionAddEventMaintenanceFragmentToSavedEventFragment()
        ...
        val modalityItems = viewModel.fetchMaintenanceModalities()
        ...
        val contextItems = viewModel.fetchContexts()
        for (item in contextItems) {
            contextList.add(item)
        }
        val contextAdapter =
            ArrayAdapter(requireContext(), R.layout.item_dropdown_list, contextList)
        (binding.dropDownEventContext as? AutoCompleteTextView)?.setAdapter(contextAdapter)
